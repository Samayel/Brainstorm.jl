...
function test_pi_machin()
    ...
    with_bigfloat_precision(40000) do
        @test all([euler(d)           - trunc(BigInt, pi*(big(10)^d)) for d in (1, 10, 100, 1000, 10000)] .== 0)
        ...
        @test all([hwang1997(d)       - trunc(BigInt, pi*(big(10)^d)) for d in (1, 10, 100, 1000, 10000)] .== 0)
        @test all([hwang2003(d)       - trunc(BigInt, pi*(big(10)^d)) for d in (1, 10, 100, 1000, 10000)] .== 0)
        @test all([wetherfield2004(d) - trunc(BigInt, pi*(big(10)^d)) for d in (1, 10, 100, 1000, 10000)] .== 0)
    end
end
